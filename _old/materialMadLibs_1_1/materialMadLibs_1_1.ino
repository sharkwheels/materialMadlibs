...
void playSongs(int s){
  ...
  if(tmrpcm.isPlaying()){
    pushCounter++; // maybe use this as a reset?
    tmrpcm.volume(1); // volume increases after each play it goes up to seven in the library
    Serial.println(pushCounter);
    while(tmrpcm.isPlaying());
    tmrpcm.stopPlayback();
  }

  // send a pattern trip to the LED. 
